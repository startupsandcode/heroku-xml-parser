...
def filings():
    return jsonify({'filers': list(map(lambda filer: filer.serialize(), Filer.query.all()))})

@app.route('/recipients', methods=['GET', ])
@app.route('/recipients/<string:state>', methods=['GET', ])
def recipients(state='ALL'):
    if state == 'ALL':
        return jsonify({'recipients': list(map(lambda recipient: recipient.serialize(), Recipient.query.all()))})
    else:
        state = state.upper()
        return jsonify({'recipients': list(map(lambda recipient: recipient.serialize(), Recipient.query.filter(Recipient.state==state).all()))})
...
def getFilerData(filer):
    ...
    filerObj['name'] = checkIfNone(filer.find('irs:Name/irs:BusinessNameLine1',namespaces=namespaces))
    ...
    try:
        filerToAdd = Filer(filerObj['ein'],
                        filerObj['name'],
                        filerObj['addressLine1'],
                        filerObj['city'],
                        filerObj['state'],
                        filerObj['postalCode'])
...
def getRecipientData(recipient,filerEin):
    errors = []
    recipientObj = {}
    recipientObj['ein'] = checkIfNone(recipient.find('irs:EINOfRecipient',namespaces=namespaces))
    recipientObj['name'] = checkIfNone(recipient.find('irs:RecipientNameBusiness/irs:BusinessNameLine1',namespaces=namespaces))
    address = getAddress(recipient.find('irs:AddressUS',namespaces=namespaces))
    ...
    recipientObj['amountOfCashGrant'] = checkIfNone(recipient.find('irs:AmountOfCashGrant',namespaces=namespaces))
    recipientObj['purposeOfGrant'] = checkIfNone(recipient.find('irs:PurposeOfGrant',namespaces=namespaces))
    try:
        recipientToAdd = Recipient(recipientObj['ein'],
                        recipientObj['name'],
                        recipientObj['addressLine1'],
                        recipientObj['city'],
                        recipientObj['state'],
                        recipientObj['postalCode'],
                        filerEin,
...
def getAddress(addressUS):
    address = {}
    address['line1'] = checkIfNone(addressUS.find('irs:AddressLine1',namespaces=namespaces))
    address['city'] = checkIfNone(addressUS.find('irs:City',namespaces=namespaces))
    address['state'] = checkIfNone(addressUS.find('irs:State',namespaces=namespaces))
    address['postalCode'] = checkIfNone(addressUS.find('irs:ZIPCode',namespaces=namespaces))
    return address
